...
def benchmark(
    ...
    for fd in fd_list:
        for _ in range(iter_repeats):
            ...
            optimizer.maximize(init_points=0, n_iter=iter_max)
    ...
    t = time.time()
    csv_fname = f"function_benchmark-{aq_name}-{t}.csv"
    log_fname = f"function_benchmark-{aq_name}-{t}.log"
    ...
    with open(csv_fname, "w", newline='') as csvfile:
        hnames = ["function_number", "avg_entropy"]
        writer = csv.DictWriter(csvfile, fieldnames=hnames)
        
        writer.writeheader()
        
        for i in range(len(fd_list)):
            writer.writerow({"function_number":i, "avg_entropy": avg_entropy[i]})
...
def main():
    ...
    raise Exception("Har inte testat denna! Men borde fungera så länge du lägger till fler FunctionDetail-objekt i listan ovan.")
        
    benchmark(fd_list,nSamples, acqf, pbounds, nu, alpha, iter_repeats=1,init_points=3)
